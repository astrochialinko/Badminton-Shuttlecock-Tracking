...
for game in game_list:
    all_path = glob(os.path.join(parent_path, game, 'frame', '*'))
    train_path = all_path[:int(len(all_path)*0.8)]
    #train_path = all_path[int(len(all_path)*0.8):]
    for i in range(len(train_path)):
        train_path[i] = train_path[i][len(os.path.join(parent_path, game, 'frame')) + 1:]
    for p in train_path:
        labelPath = os.path.join(parent_path, game, 'ball_trajectory', p + '_ball.csv')
        ...
        y = data['Y'].values
        num = no.shape[0]
        r = os.path.join(parent_path, game, 'frame', p)
        x_data_tmp = []
        y_data_tmp = []
